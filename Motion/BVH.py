...
def save(filename, anim, names=None, frametime=1.0/60.0, order='xyz', positions=False, orients=True):
    """
    ...
    with open(filename, 'w') as f:
        ...
        f.write("%sCHANNELS 6 Xposition Yposition Zposition %s %s %s \n" %
            (t, channelmap_inv[print_order[0]], channelmap_inv[print_order[2]], channelmap_inv[print_order[1]]))
        ...
        rots = np.degrees(anim.rotations.euler(order='yxz'))
        poss = anim.positions
...
def save_joint(f, anim, names, t, i, print_order, children, positions=False):
    ...
    if not end_site:
        if positions:
            f.write("%sCHANNELS 6 Xposition Yposition Zposition %s %s %s \n" % (t,
                channelmap_inv[print_order[0]], channelmap_inv[print_order[2]], channelmap_inv[print_order[1]]))
        else:
