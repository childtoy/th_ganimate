...
class SkeletonConv(nn.Module):
    def __init__(self, neighbour_list, in_channels, out_channels, kernel_size, joint_num, stride=1, padding=0,
        ...
        self.weight_cond = torch.zeros(out_channels, 3, kernel_size)
        if bias:
            self.bias = torch.zeros(out_channels)
            self.bias_cond = torch.zeros(out_channels)
        else:
    ...
    def reset_parameters(self):
        ...
        self.weight_cond = torch.nn.init.uniform_(self.weight_cond)
        self.weight = nn.Parameter(self.weight)
        self.weight_cond = nn.Parameter(self.weight_cond)
        if self.bias is not None:
            self.bias = nn.Parameter(self.bias)
            self.bias_cond = torch.nn.init.uniform_(self.bias_cond)
            self.bias_cond = nn.Parameter(self.bias_cond)
